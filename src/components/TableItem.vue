  ...
  <tr class="tableRow" @click="collapsed = !collapsed">
    <td>
      <div class="ui progress"
        :class="(server.load > 1.4 ? 'orange' : server.load > .4 ? 'warning' : (getStatus ? 'success' : 'error'))">
        <!-- :class="{ 'success': getStatus, 'warning': server.load > .4, 'orange': server.load > 1.4 , 'error': !getStatus,}"> -->
        <div class="bar" style="width: 100%"><span> {{ getStatus ? (server.load > .4 ? "過載中*" : "運行中") : '維護中' }}
          </span>
        </div>
      </div>
    </td>
    ...
    <td>
      <div class="ui progress" :class="getProcessBarStatus(getCpuStatus)">
        <div class="bar" :style="{ 'width': `${getCpuStatus.toString()}%` }">
          {{ getStatus ? `${getCpuStatus.toString()}%` : '維護中' }}
        </div>
      </div>
    </td>
    <td>
      <div class="ui progress" :class="getProcessBarStatus(getRAMStatus)">
        <div class="bar" :style="{ 'width': `${getRAMStatus.toString()}%` }">
          {{ getStatus ? `${getRAMStatus.toString()}%` : '維護中' }}
        </div>
      </div>
    </td>
    <td>
      <div class="ui progress" :class="getProcessBarStatus(getHDDStatus)">
        <div class="bar" :style="{ 'width': `${getHDDStatus.toString()}%` }">
          {{ getStatus ? `${getHDDStatus.toString()}%` : '維護中' }}
        </div>
      </div>
    </td>
  </tr>
  <tr class="expandRow">
    <td colspan="12">
      <div :class="{ collapsed }" :style="{ 'max-height': getStatus ? '' : '0' }">
        <div id="expand_mem">RAM: {{
            getStatus ? `${expandRowByteConvert(server.memory_used * 1024)} / ${expandRowByteConvert(server.memory_total *
              1024)}` : '–'
        }}
        </div>
        ...
        <div id="expand_hdd">DISK: {{
            getStatus ? `${expandRowByteConvert(server.hdd_used * 1024 * 1024)} / ${expandRowByteConvert(server.hdd_total
              * 1024 * 1024)}` : '–'
        }}
        </div>
